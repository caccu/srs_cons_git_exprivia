...
- **2026-09-25** — **Riscontro CSI alle domande di sviluppo** ([[wiki/sources/2026-09-25-riscontro-csi-domande-sviluppo\|fonte]]). (1) **Gestione Deleghe fuori perimetro anche lato BE**: il backend riceve solo il CF del delegato. Si chiude il conflict del 22/09 (WSDL mai implementato). (2) **SOAP in ingresso `/services/ConsprefService` mantenuto** per le integrazioni esistenti; le aziende federate restano sulla canalità AS-IS e l'API Manager serve solo i nuovi fruitori. Chiude la lacuna L1 di [[wiki/docs/adr/ADR-020-lis-integrazione-be-esistente\|ADR-020]]. (3) **CDU-06: scarico dell'informativa anche per l'Operatore**. Nuovo lavoro FE nella Webapp Operatore, [[wiki/docs/adr/ADR-022-cdu-06-scarico-informativa-operatore\|ADR-022]]. (4) Servizi `/informativa/*`: **ASR 904 (San Luigi) da ripristinare**, esclusione da ribasare sugli endpoint dismessi, FE Operatore da adeguare ai servizi nuovi se compatibile. `modifyToRevoked` resta senza risposta. (5) In verifica CSI: codici 502/409/503 e conferma formale degli architetti sul trust del gateway. Tracker: [[wiki/analyses/analysis-2026-05-14-punti-aperti-csi\|Punti Aperti CSI]] §9.
