---
{"dg-publish":true,"permalink":"/wiki/raw/pianificazione/","dg-note-properties":{}}
---


# Pianificazione Progetto Gestione Consensi
...
- Design DDL Schema TO-BE: 25+ tabelle documentate nel cap. 8, incluse le proposte evolutive obbligatorie (8.4.1–8.4.10) — es. `cons_t_traccia_servest`, `cons_t_batch_errori`, estensioni `cons_t_notifica` e `cons_r_asr_endpoint`
...
- Applicazione di tutte le proposte evolutive obbligatorie: `cons_t_traccia_servest` (req. CDU-09/10/11), estensione `cons_t_notifica` (BATCH-01/02/03), `statoallineamento` su `cons_r_asr_endpoint` (CDU-03, BATCH-03)
...
# Stima per i singoli sprint Agile

Piano Sprint Agile (10 sprint × 2 settimane). Ogni sprint copre backend + frontend in parallelo. La stima in Story Points (SP) adotta una scala dove 1 SP = 4 ore di lavoro di un singolo sviluppatore.
...
Nota: DBaaS Nivola e credenziali IRIS/AURA hanno latenze esterne imprevedibili. Vanno avviate nel giorno 1.
...
| DDL TO-BE base: 12 tabelle lookup (`consd_`, `consdf_`) | DB | 5 |
...
| JSON/JSONB | json in PG9 limitato → PG17 ricco di funzioni | Rivalutare query che usano operatori `->`, `->>` |
...
1. Stop applicazione (Kubernetes: `kubectl scale deploy/gestione-consensi-backend --replicas=0`)
2. Dump finale dell'istanza PG9 PROD (delta dall'ultimo dump)
3. Restore incrementale su PG17 PROD
4. Aggiornamento Kubernetes Secret con nuova stringa JDBC PG17
5. Restart (`--replicas=2`), verifica readinessProbe
6. Smoke test: login Cittadino, rilascio consenso, verifica `cons_t_notifica`
7. Monitoraggio ELK per 30 minuti post-cutover
